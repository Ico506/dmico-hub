/* ─────────────────────────────────────────────────────────────
   ...
   Writes go straight to the same kv keys the bot reads (mood_data,
   journal_data, reflections_data) so they save instantly.
   ───────────────────────────────────────────────────────────── */
...
window.renderLife = async function (container, sb) {
  ...
  moodsEl.innerHTML = MOODS.map(([e, r, lbl]) =>
    `<button class="mood" data-r="${r}" title="${lbl}">${e}</button>`).join("");
  ...
  async function saveMood(rating) {
    ...
    if (entry) { entry.rating = rating; if (note) entry.word = note; entry.timestamp = botTimestamp(); }
      ...
      data.entries.push({ date: todayISO, month: monthKey, rating, word: note || null, timestamp: botTimestamp() });
    }
    data.pending_message_id = null;
    const ok = await window.dmicoKvSet("mood_data", data);
    moodMsg.hidden = false;
    moodMsg.textContent = ok ? "Mood saved." : "Couldn't save — try again.";
  ...
  function showSuggestion(rating, entries) {
    ...
    // gentle note if recent run of low moods
  ...
  function renderJournal() {
    jEl.innerHTML = jPrompts.map((p, i) =>
      `<div class="q"><label>${esc(p)}</label><textarea data-i="${i}"></textarea></div>`).join("");
  }
  renderJournal();
  document.getElementById("life-j-shuffle").addEventListener("click", () => {
    jPrompts = shuffle(JOURNAL_PROMPTS).slice(0, 3); renderJournal();
  });
  document.getElementById("life-j-save").addEventListener("click", async () => {
    const items = [];
    jEl.querySelectorAll("textarea").forEach((t) => {
      const ans = t.value.trim();
      if (ans) items.push({ prompt: jPrompts[+t.dataset.i], answer: ans });
    });
    ...
    data.entries = Array.isArray(data.entries) ? data.entries : [];
    data.entries.push({ content, items, author: "Damico (hub)", timestamp: botTimestamp(), month: monthKey, day: dayLabel });
    const ok = await window.dmicoKvSet("journal_data", data);
    jMsg.hidden = false;
    jMsg.textContent = ok ? "Journal entry saved. ✨" : "Couldn't save — try again.";
    if (ok) { jEl.querySelectorAll("textarea").forEach((t) => (t.value = "")); renderJournalHistory(data.entries); }
  });
  ...
  function renderReflect() {
    rEl.innerHTML = rQs.map((q, i) =>
      `<div class="q"><label>${esc(q)}</label><textarea data-i="${i}"></textarea></div>`).join("");
  }
  renderReflect();
  document.getElementById("life-r-shuffle").addEventListener("click", () => {
    rQs = shuffle(REFLECT_QUESTIONS).slice(0, 4); renderReflect();
  });
  document.getElementById("life-r-save").addEventListener("click", async () => {
    ...
    rEl.querySelectorAll("textarea").forEach((t, idx) => {
      const ans = t.value.trim();
      if (ans) { answers[String(idx)] = { prompt: rQs[+t.dataset.i], answer: ans, timestamp: new Date().toISOString() }; n++; }
    });
    ...
    rMsg.hidden = false;
    rMsg.textContent = ok ? "Reflection saved. 🌱" : "Couldn't save — try again.";
    if (ok) { rEl.querySelectorAll("textarea").forEach((t) => (t.value = "")); renderReflectHistory(data.daily); }
  });
  ...
  renderTrend(moodData?.entries || []);
  renderJournalHistory(journalData?.entries || []);
  renderReflectHistory(reflectData?.daily || {});
};
